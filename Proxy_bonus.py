...
class Proxy_bonus:
    def __init__(self, upload_rate, download_rate, port=None, change_rate=False):
        ...
        Thread(target=self.__change_rate__).start()

    def __change_rate__(self, time_wait=15):
        cnt = 0
        while self.change_rate:
            time.sleep(time_wait)
            self.upload_rate = self.upload_rate / 2
            print("Rate dropped to", self.upload_rate)
            time.sleep(time_wait)
            self.upload_rate = self.upload_rate * 2
            print("Rate back to normal!")
            cnt += 1
            if cnt == 3:
                break

    def add_delay(self, delay=3):
        self.delay = delay
