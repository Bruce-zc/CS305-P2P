...
if __name__ == '__main__':
    ...
    A = PClient(tracker_address, upload_rate=400000, download_rate=100000)
    B = PClient(tracker_address, upload_rate=400000, download_rate=100000)
    C = PClient(tracker_address, upload_rate=400000, download_rate=100000)
    D = PClient(tracker_address, upload_rate=400000, download_rate=100000)
    E = PClient(tracker_address, upload_rate=400000, download_rate=100000)
    ...
    with open("../test_files/bg.png", "rb") as bg:
        ...
        for i in files:
            if files[i] != bs:
                # print(f"client:{i} files:{files[i]}")
                # print(f"origin: {bs}")
                raise Exception()
    ...
    # SC model
    server = Server(upload_rate=400000, download_rate=1000000, port=10087)
    server.start()
    ...
    #     time_middle = time.time_ns()
    #     client_download(c)
        # print(f"c {i} consume time: {(time.time_ns() - time_middle) * 1e-9}")
    #
    ...
    for t in threads:
        t.join()
        # print(f"thread ends time: {(time.time_ns() - time_start) * 1e-9}")
